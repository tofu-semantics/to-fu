...
nv.models.scatterChart = function() {
  ...
  function chart(selection) {
    selection.each(function(data) {
      ...
      shapeLegendData = data['shapeLegendData']
      ...
      gEnter.append('g').attr('class', 'nv-legendWrap');
      gEnter.append('g').attr('class', 'nv-controlsWrap');
      gEnter.append('g').attr('class', 'nv-shapesWrap');
    ...
    var availableWidth = (width  || parseInt(selection.style('width')) || 960)
                     - margin.left - margin.right
    var gwrap = d3.select(selection[0][0].node).select('.nv-scatterChart').select('g').select('.nv-shapesWrap')
   
    var g = gwrap.attr('transform','translate(' + (availableWidth/10) + ',' + (-margin.top) + ')').select('g')
    console.debug(g)
    if (!(g[0][0])){
        g = gwrap.append('g')
        .attr('transform','translate(' + (availableWidth/5) + ',10)');
    }

    var g_sel = g.selectAll('path')
        .data(selection[0][0].node.__data__.shapeLegendData) // this is horrifically ugly

    var enter_sel = g_sel.enter()
    g_sel.exit().remove()

    enter_sel.append('path')
        .attr('d', function (d) {console.debug(d.value);return d3.svg.symbol().type(d.value).size(80)()})
        .attr('transform',function (d,i) { return 'translate(' + (i * 100) + ',0)'})

    enter_sel.append('text')
        .attr('transform',function (d,i) { return 'translate(' + (i * 100 + 10) + ',0)'})
